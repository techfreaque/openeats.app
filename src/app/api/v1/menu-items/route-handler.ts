...
> = async ({ data, user }) => {
  try {
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : "Failed to retrieve menu items",
...
> = async ({ data, user }) => {
  try {
    ...
    if (data.published !== null && data.published !== undefined) {
      filteredItems = filteredItems.filter(item => item.published === data.published);
    }
    
    if (data.minPrice !== null && data.minPrice !== undefined) {
      filteredItems = filteredItems.filter(item => item.price >= data.minPrice!);
    }
    
    if (data.maxPrice !== null && data.maxPrice !== undefined) {
      filteredItems = filteredItems.filter(item => item.price <= data.maxPrice!);
    }
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : "Failed to search menu items",
      errorType: ErrorResponseTypes.INTERNAL_ERROR,
    };
  }
};
