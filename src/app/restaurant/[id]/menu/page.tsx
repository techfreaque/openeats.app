...
import { AlertCircle, Filter, Loader2, Search, SlidersHorizontal, Tag } from "lucide-react";
import Link from "next/link";
import { useParams } from "next/navigation";
import { useTranslation } from "next-vibe/i18n";
import { cn } from "next-vibe/shared/utils/utils";
import { Badge, Button, Checkbox, Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, DialogTrigger, Input, Label, RadioGroup, RadioGroupItem, Sheet, SheetContent, SheetDescription, SheetFooter, SheetHeader, SheetTitle, SheetTrigger } from "next-vibe-ui/ui";
...
type DietaryFilter = "vegetarian" | "vegan" | "glutenFree" | "dairyFree" | "spicy" | "organic";
...
export default function RestaurantMenuPage(): JSX.Element | null {
  ...
  const filteredMenuItems = menuItems.filter((item) => {
    ...
    if (priceRange === "10to20" && (item.price < 10 || item.price > 20)) return false;
    if (priceRange === "over20" && item.price <= 20) return false;
    ...
    if (dietaryFilters.length > 0) {
      ...
      const matchesDietary = dietaryFilters.some(filter => {
        switch(filter) {
          case "vegetarian": return itemDescription.includes("vegetarian") || !itemDescription.includes("meat");
          case "vegan": return itemDescription.includes("vegan");
          case "glutenFree": return itemDescription.includes("gluten-free") || itemDescription.includes("gluten free");
          case "dairyFree": return itemDescription.includes("dairy-free") || itemDescription.includes("dairy free");
          case "spicy": return itemDescription.includes("spicy") || itemDescription.includes("hot");
          case "organic": return itemDescription.includes("organic");
  ...
  const itemsByCategory = categories.map((category) => ({
    category,
    items: filteredMenuItems.filter((item) => item.category?.id === category.id),
  }));
      ...
      <div className="bg-muted py-8">
        <div className="container px-4 md:px-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="text-3xl font-bold mb-2">{restaurant?.name} - {t("restaurant.menu.title", "Our Menu")}</h1>
              <p className="text-muted-foreground max-w-2xl">
                {t("restaurant.menu.subtitle", "Explore our delicious offerings and place your order")}
              </p>
            </div>
      ...
      <div className="container px-4 md:px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <Input
              placeholder={t("restaurant.menu.searchPlaceholder", "Search menu items...")}
              className="pl-10"
              value={searchQuery}
          ...
          <Sheet open={showFiltersSheet} onOpenChange={setShowFiltersSheet}>
            ...
            <SheetContent>
              <SheetHeader>
                <SheetTitle>{t("restaurant.menu.filterOptions", "Filter Options")}</SheetTitle>
                <SheetDescription>
                  {t("restaurant.menu.filterDescription", "Customize your menu view with these filters.")}
                </SheetDescription>
              </SheetHeader>
              <div className="py-6 space-y-6">
                <div className="space-y-3">
                  <h3 className="text-sm font-medium">{t("restaurant.menu.dietaryPreferences", "Dietary Preferences")}</h3>
                  <div className="space-y-2">
                    <div className="flex items-center">
                      ...
                      />
                      <Label htmlFor="vegetarian" className="ml-2">{t("restaurant.menu.vegetarian", "Vegetarian")}</Label>
                    </div>
                    <div className="flex items-center">
                      <Checkbox
                        id="vegan"
                      ...
                      />
                      <Label htmlFor="vegan" className="ml-2">{t("restaurant.menu.vegan", "Vegan")}</Label>
                    </div>
                    <div className="flex items-center">
                      <Checkbox
                        id="glutenFree"
                      ...
                      />
                      <Label htmlFor="glutenFree" className="ml-2">{t("restaurant.menu.glutenFree", "Gluten Free")}</Label>
                    </div>
                    <div className="flex items-center">
                      <Checkbox
                        id="dairyFree"
                      ...
                      />
                      <Label htmlFor="dairyFree" className="ml-2">{t("restaurant.menu.dairyFree", "Dairy Free")}</Label>
                    </div>
                    <div className="flex items-center">
                      <Checkbox
                        id="spicy"
                      ...
                      />
                      <Label htmlFor="spicy" className="ml-2">{t("restaurant.menu.spicy", "Spicy")}</Label>
                    </div>
                    <div className="flex items-center">
                      <Checkbox
                        id="organic"
                      ...
                      />
                      <Label htmlFor="organic" className="ml-2">{t("restaurant.menu.organic", "Organic")}</Label>
                    </div>
                  </div>
                </div>

                <div className="space-y-3">
                  <h3 className="text-sm font-medium">{t("restaurant.menu.priceRange", "Price Range")}</h3>
                  <RadioGroup value={priceRange} onValueChange={(value) => setPriceRange(value as PriceRange)}>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="all" id="all" />
                      <Label htmlFor="all">{t("restaurant.menu.allPrices", "All Prices")}</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="under10" id="under10" />
                      <Label htmlFor="under10">{t("restaurant.menu.under10", "Under $10")}</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="10to20" id="10to20" />
                      <Label htmlFor="10to20">{t("restaurant.menu.10to20", "$10 to $20")}</Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="over20" id="over20" />
                      <Label htmlFor="over20">{t("restaurant.menu.over20", "Over $20")}</Label>
                    </div>
                  </RadioGroup>
                </div>
              </div>
        ...
        {featuredItems.length > 0 && !searchQuery && dietaryFilters.length === 0 && priceRange === "all" && !activeCategory && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold mb-6">{t("restaurant.menu.featuredItems", "Featured Items")}</h2>
          ...
          {itemsByCategory.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-muted-foreground">
                {t("restaurant.menu.noItemsFound", "No menu items found matching your filters.")}
              </p>
              <Button
                variant="outline"
                className="mt-4"
