...
>(
  ...
  if (!example) {
    throw new Error(
      `Example "${exampleKey}" not found for endpoint "${endpoint.path.join("/")}"`,
    );
  }

  return {
    request: example.request as TRequest,
    response: example.response as TResponse,
    urlParams: example.urlParams as TUrlVariables,
  };
}
