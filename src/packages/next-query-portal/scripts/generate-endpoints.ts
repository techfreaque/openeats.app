 ...
 *
 * This script runs at build time (or on dev) to:
 * 1) Scan "src/app/api" for subdirectories containing "definition.ts".
 * 2) Generate "src/app/api/generated/endpoints.ts" with static imports.
 */
...
export function generateEndpoints(rootDir: string): void {
  try {
    ...
    console.error(
      "❌ [next-query-portal] Failed to generate endpoints:",
      error,
    );
  }
}
...
import type { ApiSection } from "next-query-portal/shared";
